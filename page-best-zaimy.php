<?php get_header() ?>
<main data-type="creditcard" class="page-best">
	<style>
		.special_hide,
		.header__currency {
	...
	<div class="container special_hide">
		<nav aria-label="breadcrumb" class="horizontal__scroll">
			<ol class="breadcrumb horizontal__scroll-container">
				...
				<li class="breadcrumb-item active" aria-current="page">Лучшие предложения</li>
			</ol>
		</nav>
	</div>
	<!-- page header -->
	<div class="page__heading mb-sm-4 special_hide">
		<div class="container">
			<div class="page__heading-top d-flex justify-content-between align-items-center">
				<div>
					<h1 class="page__heading-title mb-0">Лучшие предложения по займам</h1>
				</div>
			...
			<div class="page__nav-container nav-tabs">
				<div class="horizontal__scroll">
					<div class="horizontal__scroll-container">
						<a href="<?php echo get_page_link(5331) ?>" class="nav-link active">Займы</a>
						<a href="<?php echo get_page_link(5333) ?>" class="nav-link">Кредиты</a>
						<a href="<?php echo get_page_link(5339) ?>" class="nav-link">Кредитные карты</a>
						<a href="<?php echo get_page_link(5335) ?>" class="nav-link">Карты рассрочки</a>
						<a href="<?php echo get_page_link(5337) ?>" class="nav-link">Дебетовые карты</a>
					</div>
				</div>
				...
				<div class="col-12">
					<div class="credits__list">
						...
						<div class="d-flex flex-wrap justify-content-between align-items-center mt-0 mt-md-5 mt-lg-0 mb-3">
							<div class="h2 mt-5 mb-4 mt-md-0 mb-md-0 special_hide"><span class="variants_count"><?= count(get_field('best-products')) ?></span> лучших займов</div>
							<div class="credits__list-dropdown dropdown mb-3 mb-md-0 col-12 col-md-5 col-lg-3 px-0">
								<select name="" class="styledSelect best-order-select">
									...
									<option value="z_sum" <?= ($order == 'z_sum') ? $selected : ''; ?>>По сумме займа</option>
									<option value="z_stavka" <?= ($order == 'z_stavka') ? $selected : ''; ?>>По процентной ставке</option>
						...
						<div class="best_offers">
							...
							<div class="best_banner">
								<div class="best_banner_logo">
									<img src="/wp-content/themes/finbank_theme/img/best_banner.png" alt="Посмотреть все">
									<div class="best_banner_text">
										<div class="best_banner_title">Не можете найти подходящее предложение?</div>
										Посмотрите разделы на сайте
									</div>
								</div>

								<?php if (have_rows('best-menu')): ?>
									<div class="best_banner_menu">
										<div class="best_banner_menu_title">
							...
							switch ($order) {
								...
								case 'z_sum':
									...
									break;
								case 'z_stavka':
									$args['meta_key'] = 'z_stavka';
									$args['orderby'] = array('meta_value_num' => 'asc', 'name' => 'desc');
									$args['order'] = 'ASC';
									break;
								
								default:
									$args['meta_key'] = 'order_priority';
							...
							if ($query->have_posts()):
								...
								while ($query->have_posts()): $query->the_post();
									...
									<div class="best_offer position-relative" style="order: <?= $io; ?>">
										<div class="best_offer_logo_block">
											<div class="best_offer_logo">
												<img src="<?= get_field('z_organization_logo', $best_id); ?>" alt="
												<?
												$bank_id = get_field('z_organization_logo', $best_id, false);
												$bank_alt = get_post_meta($bank_id, '_wp_attachment_image_alt', true);
												echo $bank_alt;
												?>">
												<div class="best_offer_meta">
													<div class="best_offer_name"><?= get_field('z_organization_name', $best_id); ?></div>
													...
													<div class="d-none d-sm-flex align-items-center">
														...
														<div class="position-relative card__icon d-flex align-items-center mr-3">
															<div class="mr-2"><a href="<?php the_permalink($best_id) ?>#comments" data-target="comments" class="stretched-link"><svg width="18" height="17" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 17" xml:space="preserve">
																		<use xlink:href="<?php bloginfo('template_url'); ?>/img/icons.svg#commentLine" x="0" y="0"></use>
																	</svg></a></div>
															<?php $comments_count = wp_count_comments($best_id);
										...
										<?php endif; ?>
										<div class="best_offer_params">
											<div class="best_offer_param">
												<div class="best_offer_param_label">Сумма</div>
												<div class="best_offer_param_value"><?= get_field('z_sum', $best_id); ?> ₽</div>
											</div>
											<div class="best_offer_param">
												<div class="best_offer_param_label">Cтавка</div>
												<div class="best_offer_param_value"><?= get_field('z_stavka', $best_id); ?>%</div>
											</div>
											<div class="best_offer_param">
												<div class="best_offer_param_label">Срок </div>
												<div class="best_offer_param_value">до <?= get_field('z_time', $best_id); ?> дней</div>
											</div>
										</div>
										<div class="best_offer_button">
											<?php
											$encoded_link = get_field('card_bank_link', $best_id);
											$card_bank_link = base64_encode($encoded_link);
											?>

											<?php if ($card_bank_link): ?>
												<span
													class="link-data btn btn-primary btn-block font-weight-normal best_offer_btn stretched-link"
													data-link="<?= esc_attr($card_bank_link); ?>"
													onclick="<?= get_metrika_for_best_pages(get_field('card_bank_link', $best_id)); ?> return true;">
													Получить деньги
												</span>

											<?php else: ?>
												<span data-popap-apply-id="<?php echo esc_attr(get_the_ID()); ?>"
													onclick="<?= get_metrika_for_best_pages(false); ?> return true;"
													class="btn btn-primary btn-block font-weight-normal best_offer_btn stretched-link">
													Подобрать
												</span>
