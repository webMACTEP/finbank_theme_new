...
function code_type_4v2($atts)
{
    ...
    foreach ($idArray as $id) {
        ...
        $img = get_field('bank_logo', $bank_choice);
        if ($img) {
            $html .= '<img src="' . esc_url($img) . '" alt="' . esc_attr(get_the_title($id)) . '" />';
        }
        $html .= '</div>
                  <div>' . esc_html(get_the_title($id)) . '</div>
                </div>
                <div class="code4data">
                    <div class="d-flex align-center"><div><img src="/codes/heart.svg" alt="' . esc_attr__('Likes', 'text-domain') . '" /></div><div>' . esc_html($likes) . '</div></div>
                    <div class="d-flex align-center"><div><img src="/codes/feeds.svg" alt="' . esc_attr__('Comments', 'text-domain') . '" /></div><div>' . esc_html($approved_comments) . '</div></div>
                    <div class="d-flex align-center"><div><img src="/codes/star.svg" alt="' . esc_attr__('Rating', 'text-domain') . '" /></div><div>' . esc_html((int) $meta['ratings_average'][0]) . '</div></div>
                </div>';
        ...
        switch ($type) {
            case 'credit_card':
                ...
                if (intval($meta['card_stavka'][0]) > 0) {
                    $html .= '<div class="code4big">От ' . intval($meta['card_stavka'][0]) . ' %</div>';
                }
                break;
